...
#pragma mark --- 修改点  修改人Chen
        /****************************原始 *************  为了兼容视图被临时修改，容易出错，越界，或找不到而崩溃
   NSIndexPath *path = [NSIndexPath indexPathForItem:currentIndex+1 inSection:self.currentSection];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(UIView *)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"frame"]) {
        ...
        if (_scrollView.subviews.count<_currentImageIndex) {
            index = 0 ;
        }
...
#pragma mark --- 修改点   2
         ...
         *********************************/
        NSIndexPath *path = [NSIndexPath indexPathForItem:self.currentImageIndex+1 inSection:self.currentSection];
