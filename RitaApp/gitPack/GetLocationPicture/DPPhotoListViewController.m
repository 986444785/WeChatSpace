...
- (UIButton *)finishButton{
    if (!_finishButton) {
        ...
        UILabel * sureLable = [[UILabel alloc]initWithFrame:CGRectMake(_finishButton.frame.size.width - 60, 0, 50 , _finishButton.frame.size.height)];
        sureLable.text = @"确定";
        sureLable.textColor  =[UIColor greenColor];
        [_finishButton addSubview:sureLable];
...
- (void)showFinishButton{
    ...
    [_finishButton setTitle:[NSString stringWithFormat:@"已选%li张",(long)_selectedCount] forState:UIControlStateNormal];
}
...
//    cell.imageView.image = [UIImage imageWithCGImage:[_assetList[indexPath.row] thumbnail]];
  cell.imageView.image = [UIImage imageWithCGImage:[_assetList[indexPath.row] aspectRatioThumbnail]];
